...
export default function ChatPage() {
    ...
    async function loadIncomingRequests() {
      try {
        ...
        const formattedRequests = requests.map((req) => ({
          ...
          avatar: req.sender_username?.charAt(0)?.toUpperCase() || "?",
          color: "#6c5ce7",
          online: true,
  ...
  const handleAcceptRequest = async (req) => {
    try {
      ...
      setAcceptedUsers((prev) =>
        prev.includes(req.username) ? prev : [...prev, req.username],
      );
    } catch (error) {
  ...
  const handleDeclineRequest = async (reqId) => {
    try {
      await rejectChatRequest(reqId);
      setIncomingRequests((prev) => prev.filter((r) => r.id !== reqId));
    } catch (error) {
      console.error("Failed to reject request:", error);
      alert(error.message || "Failed to reject chat request");
    }
  };
  ...
  const totalAttachments = activeConv?.attachments
    ? Object.values(activeConv.attachments).reduce((sum, a) => sum + a.count, 0)
    : 0;
        ...
        <div className="tg-sidebar-header">
          <div className="tg-sidebar-header-top">
            <div className="tg-sidebar-title-row">
              <button
                className="tg-icon-btn tg-menu-btn"
              ...
              <h2 className="tg-sidebar-title">
                Messages{" "}
                <span className="tg-msg-count">({conversations.length})</span>
              </h2>
            </div>
            <div className="tg-sidebar-actions">
              ...
              >
                ...
                {incomingRequests.length > 0 && (
                  <span className="tg-request-badge">
                ...
                <div className="tg-find-results">
                  {incomingRequests.length === 0 ? (
                    <div className="tg-find-empty">
                      ...
                      <span className="tg-find-empty-sub">
                    ...
                    <>
                      ...
                      {incomingRequests.length > 1 && (
                        <div className="tg-requests-actions-bar">
                          <span className="tg-requests-count">
                            {incomingRequests.length} pending
                          </span>
                          <button
                            className="tg-accept-all-btn"
                        ...
                        <div key={req.id} className="tg-request-card">
                          <div className="tg-request-card-top">
                            ...
                            <div className="tg-find-user-info">
                              <span className="tg-find-user-name">
                                {req.name}
                              </span>
                              <span className="tg-find-user-handle">
                                {req.username}
                              </span>
                            </div>
                          </div>
                      ...
                      filteredUsers.map((user) => (
                        <div key={user.id} className="tg-find-user">
                          ...
                          <div className="tg-find-user-actions">
                            {acceptedUsers.includes(user.username) ? (
                              <button
